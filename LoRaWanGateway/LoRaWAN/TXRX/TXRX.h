...
// ----------------------------------------------------------------------------
int sendPacket(LoRaWAN* loRaWAN, uint8_t *buf, uint8_t length) {
...
//	#endif
  ...
  auto error = deserializeJson(jsonBuffer, bufPtr);
	...
	if (error) {
		String eLog = "{\"n\":\"log\",\"p\":\"T sendPacket:: ERROR Json Decode: " + String(bufPtr) + "\"}";
		Log(loRaWAN, eLog);
		return(-1);
	}
	yield();
  ...
  JsonObject root = jsonBuffer.as<JsonObject>();
	...
	if ( data != NULL ) {
		...
		String eLog = "{\"n\":\"log\",\"p\":\"T sendPacket:: ERROR: data is NULL\"}";
		Log(loRaWAN, eLog);
		return(-1);
	}
...
//	buff_index
// ----------------------------------------------------------------------------
int buildPacket(LoRaWAN* loRaWAN, uint32_t tmst, uint8_t *buff_up, struct LoraUp LoraUp, bool internal) {
	long SNR;
    int rssicorr;
	...
	//#if DUSB>=1
	if (encodedLen > 255) {
		Log(loRaWAN, "{\"n\":\"log\",\"p\":\"R buildPacket:: b64 err, len=" + String(encodedLen) + "\"}");
		return(-1);
	}
	//#endif // DUSB
	...
	buff_up[4]  = loRaWAN->gid[0];
	buff_up[5]  = loRaWAN->gid[1];
	buff_up[6]  = loRaWAN->gid[2];
	buff_up[7]  = loRaWAN->gid[3];
	buff_up[8]  = loRaWAN->gid[4];
	buff_up[9]  = loRaWAN->gid[5];
	buff_up[10] = loRaWAN->gid[6];
	buff_up[11] = loRaWAN->gid[7];
	...
	if ((j<0)) {// && ( debug>=1 ) && ( pdebug & P_RADIO )) {
		Log(loRaWAN, "{\"n\":\"log\",\"p\":\"buildPacket:: Error\"}");
	}
	//#endif
	buff_index += j;
...
int receivePacket(LoRaWAN* loRaWAN) {
	...
	// XXX Not yet supported
		...
		if (LoraUp.payLength > 0) {
         ...
         int build_index = buildPacket(loRaWAN, tmst, buff_up, LoraUp, false);
			...
			if (!sendUdp(loRaWAN, loRaWAN->ttnServer, loRaWAN->port, buff_up, build_index)) {
